classdef Tile
    properties (SetAccess = immutable, GetAccess = public)
        ...
        chOriginalTileFilename    % This is the filename of the full-reolution tile
...
%                         "linking it to the original tile it was obtained from.")
            ...
            %% 7. Get tile position, size, and resolution information, also from the filename
            % format: (resolution, X, Y, width, height)
            c1chTileInfo = regexpi(chTileFilename , '\[.+\]','match');
            c1chTileInfo = split( c1chTileInfo{1}(2:end-1) , ',');
            c1chTileInfo = cellfun(@(c) c(3:end), c1chTileInfo, 'UniformOutput',false);
            
            % X and Y coordinates from upper left corner
            obj.vdTilePosition = [str2double(c1chTileInfo{1}), str2double(c1chTileInfo{2})];
            
            % Make sure only square tiles are used
            if str2double(c1chTileInfo{3}) == str2double(c1chTileInfo{4})
                obj.dOriginalTileSideLength = str2double(c1chTileInfo{3});
                obj.dTileSideLength = obj.dOriginalTileSideLength;
            else
            ...
            % Tile resolution
            obj.dOriginalPixelSideLength = 1;%str2double(c1chTileInfo{1});
