classdef Labels
    ...
    methods (Access = public)
        ...
        function obj = Labels(varargin)
            ...
            if nargin == 1 && ( isa(varargin{1}, 'char') || isa(varargin{1}, 'string') )
                ...
                while ~feof( dFileID )
                    ...
                    % headers we're looking for and to skip the header line when filling voLabels.
                    if (iCounter == 1) && (strcmp( c1chLine{1}, 'Select column' ))
                        ...
                        continue
                    elseif (iCounter == 1) && ~(strcmp( c1chLine{1}, 'Select column' ))
        ...
        function tLabelInfo = ExportToTable(obj, NameValueArgs)
            ...
            vdMuationCount = nan(dNumLabels,1);
            vsSourceFilePath = strings(dNumLabels,1);
            
            for iLabel = 1:dNumLabels                
                vsPatientID(iLabel) = string(obj.voLabels(iLabel).chPatientID);
                vdMuationCount(iLabel) = obj.voLabels(iLabel).dMutationCount;  
                vsSourceFilePath(iLabel) = string(obj.voLabels(iLabel).chInfoSourceFilepath);
            end
            
            vbClass = (vdMuationCount > (NameValueArgs.dMegabasesPerExome * NameValueArgs.dTMBCuttoff));
            tLabelInfo = table(vsPatientID, vdMuationCount, vbClass, vsSourceFilePath);
        end
        ...
        function [vdMuationCount, vbClass, tLabelInfo] = GetLabelForSlides(obj, vsSlideNames, NameValueArgs)
            ...
            for iSlide = 1:length(vsSlideNames)
                ...
                vdMuationCount(iSlide) = tAllLabelInfo.vdMuationCount(dLabelIdx);
