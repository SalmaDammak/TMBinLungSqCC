classdef Tiles
    ...
    methods
        ...
        function obj = FillFromDir(obj, chTileDir)
            ...
            for i = 1 : length(stTilePaths)
                ...
                try
                    obj.voTiles = [obj.voTiles, Tile(chTileDir, stTilePaths(i).name)]; % TO DO: preassign vector
        ...
        function [oTrainTiles , oTestTiles, dActualFractionOfGroupsForTraining, dFractionOfSamplesForTraining,...
            ...
            arguments
                ...
                NameValueArgs.dAcceptablePositiveMin = 0;
                NameValueArgs.dAcceptablePositiveMax = 1;
            end
            ...
            if iNumTrainGroups == 0 || iNumTestGroups == 0
                error('Tiles:SplitIntoTrainAndTestPatients:NotEnoughPatients',...
                    ['Not enough patients exist for a train-test split. The tiles provided ',...
                    'refer to a total of ', num2str(iNumGroups), ' patient(s).']);
            end
            
            if NameValueArgs.dAcceptablePositiveMin > NameValueArgs.dAcceptablePositiveMax
                error('dAcceptablePositiveMin must be < dAcceptablePositiveMax')
            elseif NameValueArgs.dAcceptablePositiveMin == NameValueArgs.dAcceptablePositiveMax
                error('dAcceptablePositiveMin cannot be equal to dAcceptablePositiveMax')
            end
            
            if (obj.GetPercentOfClass(1) <= NameValueArgs.dAcceptablePositiveMin) ||...
                    (obj.GetPercentOfClass(1) >= NameValueArgs.dAcceptablePositiveMax)
                chErrorMsg = ['The percentage of the input object that is positive is ',...
                    num2str(obj.GetPercentOfClass(1)*100,'%.f'),'%%. Which is outside the [min,max] boundaries entered: [',...
                    num2str(NameValueArgs.dAcceptablePositiveMin*100,'%.f'),'%%, ',...
                    num2str(NameValueArgs.dAcceptablePositiveMax*100,'%.f'),'%%].'];
                error('Tiles:SplitIntoTrainAndTestPatients:OverallPercentPositiveOutOfBounds',chErrorMsg);
            end
                                    
            % Get a new set until one is made that matches the required %
            % positive bounds
            dPercentPositiveTrain = NameValueArgs.dAcceptablePositiveMin;
            dPercentPositiveTest = NameValueArgs.dAcceptablePositiveMin;
                                    
            while dPercentPositiveTrain <= NameValueArgs.dAcceptablePositiveMin...
                    || dPercentPositiveTrain >= NameValueArgs.dAcceptablePositiveMax...
                    || dPercentPositiveTest <= NameValueArgs.dAcceptablePositiveMin...
                    || dPercentPositiveTest >= NameValueArgs.dAcceptablePositiveMax
            ...
            dPercentPositiveTrain = oTrainTiles.GetPercentOfClass(1);
            dPercentPositiveTest = oTestTiles.GetPercentOfClass(1);
            end
    ...
    methods (Static)
        ...
        function [bIsThereALeak, c1chLeakedGroups] = CheckForLeakageIntoTestSetFromTables(tTrainTiles, tTestTiles, NameValueArgs)
            ...
            c1chTrainSplit = cellfun(@(c) strsplit(c,'\'),c1chTrainTilePaths,'UniformOutput',false);
            c1chTestSplit = cellfun(@(c) strsplit(c,'\'),c1chTestTilePaths,'UniformOutput',false);
            
            % Get the tile filename
            c1chTrainTileFilenames = cellfun(@(c) c{1,end},c1chTrainSplit,'UniformOutput',false);
            c1chTestTileFilenames = cellfun(@(c) c{1,end},c1chTestSplit,'UniformOutput',false);
            
            c1chTrainGroups = cellfun(@(c) regexp(c, 'TCGA-(\w\w-\w\w\w\w)', 'tokens') ,c1chTrainTileFilenames);
            c1chTrainGroups = [c1chTrainGroups{:}]; % This reshapes them to be c1ch instead of c1c1ch
            
            c1chTestGroups = cellfun(@(c) regexp(c, 'TCGA-(\w\w-\w\w\w\w)', 'tokens') ,c1chTestTileFilenames);
        ...
        function MakeCollagesOfTiles(c1chListOfTilePaths, dMaxNumTilePerCollage, chSavingsBasePath, nameValueArgs)
            ...
            arguments
                ...
                nameValueArgs.c1chLabelsToOverlay = {}
            end
            ...
            if length(c1chListOfTilePaths) > dMaxNumTilePerCollage
                ...
                for i = 0:floor(length(c1chListOfTilePaths)/dMaxNumTilePerCollage)
                    ...
                    c1chCurrentImages = c1chListOfTilePaths(dStart:dEnd);
                    
                    % If we need to label the images, we have load them and label
                    % them individually first
                    if ~isempty(nameValueArgs.c1chLabelsToOverlay)
                        
                        c1chLabelsInCollage = nameValueArgs.c1chLabelsToOverlay(dStart:dEnd);
                        c1m3dImagesInCollage = cell(1,length(c1chCurrentImages));
                        
                        for iCollageIndex = 1:length(c1chCurrentImages)
                            m3dLabelledImage = imread(c1chCurrentImages{iCollageIndex});
                            m3dLabelledImage = insertText(m3dLabelledImage, [1,1],c1chLabelsInCollage{iCollageIndex},...
                                'FontSize',25,'BoxColor','black','BoxOpacity',1,'TextColor','white');
                            c1m3dImagesInCollage{iCollageIndex}= m3dLabelledImage;
                        end
                        
                        % Create a collage with a border between images
                        oMontage = montage(c1m3dImagesInCollage,'BorderSize',[1 1]);
                    else
                        
                        % Create a collage with a border between images
                        oMontage = montage(c1chCurrentImages,'BorderSize',[1 1]);
                    end
                ...
                if ~isempty(nameValueArgs.c1chLabelsToOverlay)
                    ...
                    c1chLabelsInCollage = nameValueArgs.c1chLabelsToOverlay;
                    c1m3dImagesInCollage = cell(1,length(c1chListOfTilePaths));
                    
                    for iCollageIndex = 1:length(c1chListOfTilePaths)
                        m3dLabelledImage = imread(c1chListOfTilePaths(iCollageIndex));
                        m3dLabelledImage = insertText(m3dLabelledImage, [1,1],c1chLabelsInCollage(iCollageIndex),...
                            'FontSize',25,'BoxColor','green','BoxOpacity',1,'TextColor','white');
                        c1m3dImagesInCollage{iCollageIndex}= m3dLabelledImage;
                    end
                    
                    % Create a collage with a border between images
                    oMontage = montage(m3dLabelledImage,'BorderSize',[1 1]);
                else
